    ...
    try {
      ...
      try {
        ...
        if (metricsResponse.ok) {
          ...
          const agentOps = metricsData.agent_operations || {};
          setAgentMetrics({
            totalOperations: agentOps.count || 0,
            successRate: calculateSuccessRate(agentOps.by_type || {}),
            avgResponseTime: calculateAvgResponseTime(agentOps.by_type || {}),
            activeAgents: Object.keys(agentOps.by_type || {}).length,
          });
        }
      } catch (error) {
        console.warn('Could not fetch agent metrics:', error);
      }
  ...
  const defaultCapabilities: AgentCapability[] = [
    {
      ...
      description: 'Process complex financial questions with source verification',
    ...
    },
    {
      ...
      description: 'Assess credibility and trustworthiness of information sources',
      status: serviceStatus === 'connected' ? 'available' : 'unavailable'
    },
    {
      name: 'Content Generation',
      description: 'Generate comprehensive responses with proper citations',
      status: serviceStatus === 'connected' ? 'available' : 'unavailable'
    }
  ];
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium">Azure AI Agent Service</CardTitle>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-medium text-foreground">Agent Capabilities</h4>
        ...
        {serviceStatus === 'disconnected' && (
          <div className="text-xs text-red-600 bg-red-50 p-2 rounded">
            <strong>Service Unavailable:</strong> Azure AI Agent Service is not responding. 
        ...
        {serviceStatus === 'connected' && (
          <div className="text-xs text-green-600 bg-green-50 p-2 rounded">
            <strong>Service Active:</strong> Azure AI Agent Service is operational and ready to process requests.
          </div>
        )}
      </CardContent>
    </Card>
